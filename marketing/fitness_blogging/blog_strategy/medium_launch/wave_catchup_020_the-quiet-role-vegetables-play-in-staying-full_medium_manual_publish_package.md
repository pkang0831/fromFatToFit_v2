...
2. Paste the owned-site URL:
   `https://devenira.com/blog/the-quiet-role-vegetables-play-in-staying-full` — exact
   slug is `the-quiet-role-vegetables-play-in-staying-full` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
