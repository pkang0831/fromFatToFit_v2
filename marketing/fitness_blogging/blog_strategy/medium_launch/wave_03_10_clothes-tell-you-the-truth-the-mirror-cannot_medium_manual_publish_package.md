...
2. Paste the owned-site URL:
   `https://devenira.com/blog/clothes-tell-you-the-truth-the-mirror-cannot` — exact
   slug is `clothes-tell-you-the-truth-the-mirror-cannot` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
...
Why do my clothes fit better but the scale is the same? Body composition shifted; total mass didn't — and the clothes don't negotiate.
