...
2. Paste the owned-site URL:
   `https://devenira.com/blog/do-i-actually-have-to-meal-prep-to-lose-weight` — exact
   slug is `do-i-actually-have-to-meal-prep-to-lose-weight` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
