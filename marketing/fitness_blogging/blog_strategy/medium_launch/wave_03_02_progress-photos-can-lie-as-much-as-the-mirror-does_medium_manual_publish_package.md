...
2. Paste the owned-site URL:
   `https://devenira.com/blog/progress-photos-can-lie-as-much-as-the-mirror-does` — exact
   slug is `progress-photos-can-lie-as-much-as-the-mirror-does` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
...
Why progress photos don't show progress half the time has nothing to do with the body. It's lighting, posture, and last night's dinner.
