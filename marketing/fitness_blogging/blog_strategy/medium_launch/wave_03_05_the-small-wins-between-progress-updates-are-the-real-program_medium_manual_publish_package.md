...
2. Paste the owned-site URL:
   `https://devenira.com/blog/the-small-wins-between-progress-updates-are-the-real-program` — exact
   slug is `the-small-wins-between-progress-updates-are-the-real-program` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
