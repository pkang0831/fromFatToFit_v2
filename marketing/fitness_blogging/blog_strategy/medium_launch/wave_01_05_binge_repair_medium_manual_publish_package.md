...
2. Paste the owned-site URL:
   `https://devenira.com/blog/binge-repair` — exact
   slug is `binge-repair` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
...
You need fewer reasons to binge again, not a more dramatic apology tour.
