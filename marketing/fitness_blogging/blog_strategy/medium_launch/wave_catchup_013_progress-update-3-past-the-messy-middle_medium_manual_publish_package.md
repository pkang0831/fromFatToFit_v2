...
2. Paste the owned-site URL:
   `https://devenira.com/blog/progress-update-3-past-the-messy-middle` — exact
   slug is `progress-update-3-past-the-messy-middle` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
