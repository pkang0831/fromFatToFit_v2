...
2. Paste the owned-site URL:
   `https://devenira.com/blog/why-people-gain-more-back-than-they-lost` — exact
   slug is `why-people-gain-more-back-than-they-lost` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
