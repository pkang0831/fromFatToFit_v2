...
2. Paste the owned-site URL:
   `https://devenira.com/blog/the-first-month-of-maintenance-feels-nothing-like-the-diet` — exact
   slug is `the-first-month-of-maintenance-feels-nothing-like-the-diet` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
