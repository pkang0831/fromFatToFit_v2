...
2. Paste the owned-site URL:
   `https://devenira.com/blog/the-scale-can-say-normal-and-still-tell-you-nothing-useful` — exact
   slug is `the-scale-can-say-normal-and-still-tell-you-nothing-useful` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
