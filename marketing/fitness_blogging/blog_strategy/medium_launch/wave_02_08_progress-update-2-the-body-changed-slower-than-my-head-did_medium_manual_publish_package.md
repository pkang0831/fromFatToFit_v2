...
2. Paste the owned-site URL:
   `https://devenira.com/blog/progress-update-2-the-body-changed-slower-than-my-head-did` — exact
   slug is `progress-update-2-the-body-changed-slower-than-my-head-did` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
