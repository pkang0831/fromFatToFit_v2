...
2. Paste the owned-site URL:
   `https://devenira.com/blog/the-scale-lies-differently-in-the-morning-than-in-the-evening` — exact
   slug is `the-scale-lies-differently-in-the-morning-than-in-the-evening` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
