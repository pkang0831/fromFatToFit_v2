...
2. Paste the owned-site URL:
   `https://devenira.com/blog/cheat-day` — exact
   slug is `cheat-day` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
...
## SEO Title
Are Cheat Days Bad for Weight Loss? Why They Backfire
