...
2. Paste the owned-site URL:
   `https://devenira.com/blog/the-friend-who-never-diets-and-never-gains` — exact
   slug is `the-friend-who-never-diets-and-never-gains` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
