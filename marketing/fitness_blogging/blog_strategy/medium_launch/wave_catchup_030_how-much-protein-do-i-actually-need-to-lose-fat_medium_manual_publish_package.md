...
2. Paste the owned-site URL:
   `https://devenira.com/blog/how-much-protein-do-i-actually-need-to-lose-fat` — exact
   slug is `how-much-protein-do-i-actually-need-to-lose-fat` — verify against `posts.ts` (the file name's topic part is a hint, not always the exact slug)
