...
def main() -> None:
    ...
    md.append("")
    md.append("1. Review `git log v2/main..session/autonomous-3h-20260422` (6 commits, ~80 files).")
    md.append("2. Open `marketing/.../all_waves_tracker_20260422.md` for the full picture, or `qa/publish_queue_20260422.md` for the top-5-only view.")
    md.append("3. Top publish recommendation: `wave_catchup_028_why-you-stop-losing-weight-around-month-three`. Score 16, question-form title, weight-loss-plateau intent (high search volume).")
    md.append("4. Fix wave_01_01 canonical in Medium (60-sec browser fix; see `DECISIONS_REQUIRED.md` § 1).")
    md.append("5. Truly missing asset: `marketing/fitness_blogging/selected_cut/final_cuts/pkang_0011 1.jpg` referenced from wave_01_01. Restore it or update the package's Recommended asset line.")
    md.append("6. If you accept the changes, push: `git push v2 session/autonomous-3h-20260422` then open PR.")
    md.append("7. Optional: `git rm frontend/public/founder/*.bak.webp` to drop the re-encode backups (we kept them for safe revert).")
    md.append("")

    md.append("## Engagement-range forecast (30d Medium cold-start)")
    md.append("")
    er_path = ROOT / "qa/engagement_predictions_20260422.json"
    if er_path.exists():
        ...
        md.append("If you publish all 64 packages on Medium over the next 30 days,")
        md.append("the heuristic-derived 30-day aggregate range:")
        md.append("")
        md.append("| Metric | Low | High |")
        md.append("|---|---|---|")
        md.append(f"| Views | {v_lo:,} | {v_hi:,} |")
        md.append(f"| Reads | {r_lo:,} | {r_hi:,} |")
        md.append(f"| Claps | {c_lo:,} | {c_hi:,} |")
        md.append("")
        md.append("Methodology + per-package upper-bound ranking in")
        md.append("`qa/engagement_predictions_20260422.md`.")
        md.append("")
    ...
    md.append("marketing/.../medium_launch/all_waves_tracker_20260422.md  <- single source-of-truth for all 64 packages")
    ...
    md.append("qa/engagement_predictions_20260422.md                <- per-package views/reads/claps ranges")
    md.append("qa/engagement_predictions_20260422.json              <- raw range data")
    ...
    md.append("scripts/seo_retrofit/fix_duplicate_first_para_20260422.py")
    md.append("scripts/seo_retrofit/fix_stale_asset_paths_20260422.py")
    ...
    md.append("scripts/seo_retrofit/build_unified_wave_tracker_20260422.py")
    md.append("scripts/seo_retrofit/build_dashboard_20260422.py")
    md.append("scripts/seo_retrofit/predict_engagement_ranges_20260422.py")
    md.append("```")
    md.append("")
