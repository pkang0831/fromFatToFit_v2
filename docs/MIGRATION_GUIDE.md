...
Supabase SQL Editor에서 아래 SQL을 **순서대로** 복사해서 실행하세요.

---

## Step 1: Exercise Library에 MET 값 추가

```sql
-- Add MET (Metabolic Equivalent of Task) values to exercise library
...
-- Update existing exercises with MET values
...
## Step 2: Workout Logs에 칼로리 필드 추가

```sql
-- Add calories_burned field to workout_logs table
...
## Step 3: 27개 유산소 운동 추가 (MET 값 포함)

```sql
-- Add Cardio/Aerobic Exercises with MET values
INSERT INTO exercise_library (name, category, muscle_groups, form_instructions, difficulty, met_value, exercise_type) VALUES
-- Running/Jogging
...
**MET (Metabolic Equivalent)**는 운동 강도를 나타내는 표준 지표입니다:
- **1.0 MET** = 앉아서 쉬는 상태
- **3-6 MET** = 가벼운~중간 강도 (걷기, 가벼운 사이클)
- **6-9 MET** = 중간~높은 강도 (조깅, 수영)
- **9+ MET** = 매우 높은 강도 (달리기, HIIT)

이 값들은 **Compendium of Physical Activities** (과학적 표준)에서 가져왔습니다.
...
### 예시 1: 러닝 30분 (70kg 남성)
- **MET**: 9.0
- **계산**: 9.0 × 70kg × 0.5시간 = **315 kcal**

### 예시 2: 스핀 클래스 45분 (60kg 여성)
- **MET**: 8.5
- **계산**: 8.5 × 60kg × 0.75시간 = 382.5 kcal
- **여성 보정**: 382.5 × 0.9 = **344 kcal**

### 예시 3: 벤치프레스 3세트 (80kg 남성)
- **기본**: 6.0 MET × 80kg × 15분
- **작업량**: (3세트 × 10렙 × 60kg) × 0.00175 × 80kg
- **합계**: **약 120-150 kcal**
